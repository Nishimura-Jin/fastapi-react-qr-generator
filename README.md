...
**ダウンロード時のファイル名**
画像を保存するとき、ファイル名がランダムな文字列にならないよう、URLのホスト名から自動で生成するようにしました（例：`qr_example.com.png`）。あとから見てどのQRコードか分かるようにしたかったためです。
...
**FastAPIのルーティング**
`DELETE /api/history` と `DELETE /api/history/{id}` を両方定義したとき、一括削除が動かない問題がありました。FastAPIは上から順にルートを評価するため、`/api/history` を先に書かないと `/api/history/{id}` に引っ張られてしまうことが原因でした。
